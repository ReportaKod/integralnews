        ...
        return rule.custom(async (value, context) => {
          ...
          try {
            ...
            if (otherFeaturedPosts && otherFeaturedPosts.length > 0) {
              ...
              return `⚠️ Attention : ${otherFeaturedPosts.length} autre(s) article(s) ${otherFeaturedPosts.length > 1 ? 'ont' : 'a'} déjà le switch "Article à la une" activé : ${titles}${more}. Seul le plus récent sera affiché à la une.`;
            }
          } catch (error) {
